...
Your challenge for today is to complete the fill out the missing parts of the Booklist class 
in book_class.py

Then, test each method for the class Booklist!

Hint: all methods for the Booklist class go in the book_class.py script
This class and the methods are already imported in this script -- test them here for 
specific cases!
'''
...
-The method should make a book variable as a dictionary with two key/value pairs for 
title/author
-Then, the method should append this book to books attribute of the the Booklist object
  - i.e. self.books.append(book)
...
Then, add 2 books of your choice from the New York Times best sellers lists 
to nyt_bestsellers using the .add() method
...
nyt_bestsellers.add("Medium Raw", "Anthony Bourdain")

print(nyt_bestsellers.books)
'''
BONUS Part #6:
Define a display_titles() method to display all the titles of the books in an object of class 
Booklist

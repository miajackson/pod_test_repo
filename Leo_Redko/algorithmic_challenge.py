...
'''the time complexity is 0(n) because the function check_if_fruit_in_list(my_list, fruit) is linear time.
'''
...
# TODO Write the time complexity
''' the time complexity for the function- capitalize_meats_and_cheeses(meats, cheeses) is O(n) because it is also linear. Each for loop within the function is ran by the number of each item in the list. 
'''
...
# TODO Write the time complexity
''' 
The time complexity in deli_meat_challenge(meat, cheeses, sandwiches) is O(n^2). It increases exponentially. Each inner loop multiplies itself by the outer loop. 
'''
...
def contains_duplicates(my_list):
    ...
    return False
# TODO Write the time complexity
''' the function contains_duplicates has a quadratic Time complexity bc of the nested loops which is O(n^2)'''
